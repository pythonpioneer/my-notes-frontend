						...
						<Grid container className="">
							...
							>
								<button
									...
									onTouchEnd={() => {playClickAudio(audioClose);}}
								>
									Register
								</button>
							</Grid>
							...
							<Grid
								...
								className="d-flex justify-content-center"
							>
								{isLoading ? <LoggingUser theme={themeStatus} /> : <Link className="account" to="/login" style={{ color: "#A735FF", cursor: "pointer", margin: "auto" }} onTouchEnd={() => {playClickAudio(audioClose);}}>Already have an account</Link>}
							</Grid>
							...
							<Grid item xs={12} mt={2} className="d-flex justify-content-center">
								{!(errors.confirmPassword || errors.password || errors.email || errors.name)
									? <a className='account' target="_blank" rel="noreferrer" style={{ color: (themeStatus === 'dark' ? 'whitesmoke' : '#262626'), cursor: "pointer", margin: "auto" }} href='https://github.com/pythonpioneer/my-notes-frontend' onTouchEnd={() => {playClickAudio(audioClose);}}>Contribute @pythonpioneer</a>
